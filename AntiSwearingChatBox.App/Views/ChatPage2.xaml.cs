...
namespace AntiSwearingChatBox.App.Views
{
    ...
    /// </summary>
    public partial class ChatPage2 : Page
    {
        ...
        private async void ConversationList_NewChatRequested(object sender, EventArgs e)
        {
            ...
            var userDialog = new UserSelectionDialog(_userService, _currentUserId);
            userDialog.Owner = Window.GetWindow(this);
            
            var result = userDialog.ShowDialog();
            if (result == true && userDialog.SelectedUser != null)
            {
                // Check if thread already exists
                var existingThread = _chatThreadService.GetPrivateThreadBetweenUsers(_currentUserId, userDialog.SelectedUser.UserId);
                if (existingThread != null)
                {
                    // Select the existing thread
                    ConversationList_ConversationSelected(this, existingThread.ThreadId.ToString());
                    return;
                }
                ...
                {
                    Title = userDialog.SelectedUser.Username,
                    IsPrivate = true,
                    CreatedAt = DateTime.Now
                };
                ...
                {
                    ThreadId = newThread.ThreadId,
                    UserId = userDialog.SelectedUser.UserId,
                    JoinedAt = DateTime.Now
                };
                ...
                ConversationList_ConversationSelected(this, newThread.ThreadId.ToString());
                
                // Notify the other user via SignalR
                if (_connection?.State == HubConnectionState.Connected && _currentUser != null)
                {
                    await _connection.InvokeAsync("NotifyNewThread", userDialog.SelectedUser.UserId, _currentUser.Username, newThread.ThreadId);
                }
            }
        }
